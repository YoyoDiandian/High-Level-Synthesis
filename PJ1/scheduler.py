...
def schedule_ii(self, resource, delay):
    schedule = []
    resource_remain = resource[:]
    cfg = self.cfg
    for label, bb in cfg.items():
...
if __name__ == "__main__":
    ...
    func_info = {
        ...
        'return_type': 'int',
        'args': [
            {'name': 'a', 'is_array': True},
            {'name': 'b', 'is_array': True},
            {'name': 'n', 'is_array': False}
        ],
        'basic_blocks': [
            {
                'label': 'start',
                'operations': [
                    {'type':13, 'target':'i_reg', 'operands':[0]},
                    {'type':13, 'target':'c_reg', 'operands':[0]},
                    {'type':1,  'target':'temp',  'operands':['a_reg', 'b_reg']},
                    {'type':7,  'target':'next_bb',  'operands':['end']}
                ]
            }
        ]
    }
