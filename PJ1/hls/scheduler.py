...
def add_scheduling_to_cdfg(cdfg_class):
    ...
    def schedule_asap(self):
        ...
        返回：调度结果，格式为 {bb_label: [[op_indices_in_cycle_0], [op_indices_in_cycle_1], ...]}
        其中每个内部列表表示一个周期中被调度的操作索引
        """
        schedule_result = {}
        ...
        for bb_label, bb in self.cfg.items():
            ...
            while True:
                ...
                for op_type, ops in resource_groups.items():
                    # 获取该资源类型的可用数量
                    available_resources = resource[op_type]
            ...
            schedule_result[bb_label] = cycle_list
